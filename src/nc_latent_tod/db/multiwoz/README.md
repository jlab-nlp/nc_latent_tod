...
same. **We then process these with the following:**

```bash
python unflatten_ontology 2.1/ontology.json
python unflatten_ontology 2.4/ontology.json
```

These do not modify values, but set the slot names according to the schema
defined in service -> slot hierarchical form. See results from `read_multiwoz_schema()`