...
class SpeedLimitTimer:
    ...
    def __init__(self, second_per_step=0.75):
        ...
        self.seconds_per_step = second_per_step

    def step(self, seconds_per_step: float = None):
        seconds_per_step = seconds_per_step or self.seconds_per_step
        time_div = time.time() - self.record_time
        if time_div <= seconds_per_step:
            time.sleep(seconds_per_step - time_div)
        self.record_time = time.time()
